...
public interface ProductRepository extends JpaRepository<Product, Long>, JpaSpecificationExecutor<Product> {
    ...
    @Query("SELECT new com.br.personniMoveis.dto.product.get.ProductGetDto(p.productId, p.name, p.value, p.quantity, p.editable, p.imgUrl, p.description) " +
            "FROM Product p LEFT JOIN p.tags t WHERE t.tagId = :tagId")
